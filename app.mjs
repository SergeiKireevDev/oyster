...
/** Current working directory for the pi process; changing it requires a
 *  respawn, which is how "new session in another folder" works. */
let currentDir = PI_DIR;
...
function startPi() {
  ...
  console.log(`[pi-ui] spawning: ${PI_BIN} ${args.join(" ")} (cwd: ${currentDir})`);
  pi = spawn(PI_BIN, args, { cwd: currentDir, stdio: ["pipe", "pipe", "pipe"] });
...
function listSessions() {
  const dir = sessionDirFor(currentDir);
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === "GET" && url.pathname === "/events") {
    ...
    res.write(`data: ${JSON.stringify({ type: "replay_done", _server: true, piRunning: !!pi, workdir: currentDir })}\n\n`);
    sseClients.add(res);
  ...
  if (req.method === "GET" && url.pathname === "/browse") {
    const target = resolve(url.searchParams.get("path") || currentDir);
    let entries;
    try {
  ...
  if (req.method === "POST" && url.pathname === "/workdir") {
    ...
    currentDir = target;
    console.log(`[pi-ui] workdir changed to ${currentDir}, respawning pi`);
    stopPi();
    serverEvent({ type: "workdir_changed", workdir: currentDir });
    setTimeout(startPi, 300);
    json(res, 202, { workdir: currentDir });
    return;
  }

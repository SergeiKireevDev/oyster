...
export async function init(state) {
  const { listTunnels, allocateHublot, reserveHublot, recordHublotTransition, rebindHublot, restartHublotService, openTunnel, closeTunnel, closeAllTunnels, spawnHublotAgent } =
  ...
  state.piProcesses = createPiProcessLauncher({ config }); if (!state.hublotSupervisor) state.hublotSupervisor = createHublotSupervisor({ appStore, recordTransition: (id, status, options) => recordHublotTransition(state, id, status, options), restartService: (hublot) => restartHublotService(state, hublot) }); if (!state.hublotStartupReconciled) { state.hublotStartupReconciliation = await state.hublotSupervisor.reconcile({ includeOpening: true }); state.hublotStartupReconciled = true; } state.hublotSupervisor.start();

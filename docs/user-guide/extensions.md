...
A hublot is the managed tunnel behind a public live-interface widget: a `cloudflared` tunnel to a local port. Start your local service separately, then call `hublot` with `action="open"` and its `port` (1–65535). An optional `description` supplies the widget label. The tool starts only cloudflared and saves the hublot entry in SQLite; it does not allocate ports, launch agents, or provision services. Creating one automatically pins its widget. Closing or reopening any hublot manages only the tunnel, leaving the local service under your control. Legacy provisioning agents and startup-script restoration have been removed.

...
A hublot is the managed tunnel behind a public live-interface widget: a `cloudflared` tunnel to a local port. Start your local service separately, then call `hublot` with `action="open"` and its `port` (1–65535). The tool has no prompt or description argument. The tool starts only cloudflared and saves the hublot entry in SQLite; it does not allocate ports, launch agents, or provision services. Creating one automatically pins its widget. Closing or reopening a new hublot manages only the tunnel, leaving the local service under your control.

...
Only OAuth providers exposed by the configured pi SDK are offered. Pi owns provider discovery, PKCE and state validation, token exchange, refresh, and locked persistence. Oyster only presents the provider's transient browser, device-code, prompt, selection, or manual-code interactions.

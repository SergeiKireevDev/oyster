...
| — | `PI_UI_DB_PATH` | `~/.pi/agent/pi-lot-ui.sqlite` | pi-lot-ui application database |
...
`PI_UI_DB_PATH` stores pi-lot-ui-owned data such as settings, checkpoints, routines, and operation state. The pi session database is owned by the coding agent. The server refuses to start if both resolve to the same SQLite file.
...
Browser presentation settings—such as thinking visibility, carousel position, and the selected checkpoint model—remain device-local. Authentication values are never stored as general application settings.

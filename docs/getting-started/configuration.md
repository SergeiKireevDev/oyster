...
Claude Code runs in headless stream-JSON mode and uses its normal credentials (`~/.claude`, `ANTHROPIC_API_KEY`, or a configured third-party provider). Oyster does not copy pi credentials into Claude Code. The model selector exposes Claude Code's `default`, `sonnet`, `opus`, `haiku`, and `fable` aliases and applies changes through Claude's native stream control protocol. The default `acceptEdits` permission mode avoids interactive edit prompts that cannot be answered through the current web protocol; set `CLAUDE_CODE_PERMISSION_MODE=default` for a more restrictive setup or provide explicit tool policy through `CLAUDE_CODE_ARGS`. Only use `bypassPermissions` inside an appropriately isolated sandbox.

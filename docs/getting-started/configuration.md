...
Claude Code runs in headless stream-JSON mode and uses its normal credentials (`~/.claude`, `ANTHROPIC_API_KEY`, or a configured third-party provider). When Claude Code is enabled, Oyster's Credentials modal offers a dedicated Anthropic OAuth connection for it. Authenticate pi and Claude Code separately: pi stores its grant in `auth.json`, while Claude Code stores its distinct grant in `.credentials.json`. Oyster keeps that grant fresh itself: it rotates the access token about 30 minutes before expiry (well ahead of Claude Code's own five-minute refresh window, so the single-use refresh token is never contended), then restarts idle Claude Code runners so they reload the file. A runner that still reports an expired-token error triggers the same recovery immediately. Only a rejected refresh token requires re-authenticating from the Credentials modal. Other providers and API keys remain pi-only. The model selector exposes Claude Code's `default`, `sonnet`, `opus`, `haiku`, and `fable` aliases and applies changes through Claude's native stream control protocol. The default `acceptEdits` permission mode avoids interactive edit prompts that cannot be answered through the current web protocol; set `CLAUDE_CODE_PERMISSION_MODE=default` for a more restrictive setup or provide explicit tool policy through `CLAUDE_CODE_ARGS`. Only use `bypassPermissions` inside an appropriately isolated sandbox.

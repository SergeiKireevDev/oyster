...
const STALE_UPLOAD_AGE_HOURS = 24;
...
const DEFAULT_FILE_CREATE_MODE = 0o666;


const isHidden = (name) => name.startsWith(".");
const STALE_UPLOAD_AGE_MS = STALE_UPLOAD_AGE_HOURS * SECONDS_PER_MINUTE * SECONDS_PER_MINUTE * MILLISECONDS_PER_SECOND;
const MAX_EDITABLE_FILE_SIZE = 2 * BYTES_PER_KIBIBYTE * BYTES_PER_KIBIBYTE;
...
function writeUploadChunk(path, buffer, { append = false } = {}) {
  ...
  const fd = openSync(path, flags, DEFAULT_FILE_CREATE_MODE);
  try {
...
export function createFileRoutes({ state, requestContext, logger = console } = {}) {
  ...
  const forbidden = (res, path) => json(res, HTTP_FORBIDDEN, { error: `path outside the allowed roots: ${String(path ?? "")}` });
  ...
  async function readUploadChunk(req, offset) {
    ...
    try { buf = await readRawBody(req); }
      ...
      return { status: error?.code === "body_too_large" ? HTTP_CONTENT_TOO_LARGE : HTTP_BAD_REQUEST, body: { error: error?.code === "body_too_large" ? "upload chunk too large" : "upload body could not be read" } };
    }
  ...
  function sendUploadSuccess({ res, target, offset, buf, last }) {
    if (!last) return json(res, HTTP_OK, { received: offset + buf.length });
    let bytes;
    try { bytes = statSync(target).size; }
    catch { return json(res, HTTP_INTERNAL_SERVER_ERROR, { error: "upload finalization failed" }); }
    log(`[oyster] file uploaded via explorer: ${target} (${bytes} bytes)`);
    return json(res, HTTP_OK, { saved: target, bytes });
  }
  ...
  return {
    "GET /browse": (_req, res, url) => {
      ...
      catch (error) { json(res, HTTP_BAD_REQUEST, { error: `cannot read ${target}: ${error.message}` }); return; }
    ...
    "GET /file-download": (_req, res, url) => {
      ...
      try {
        ...
        if (fd !== undefined) try { closeSync(fd); } catch {}
        json(res, HTTP_NOT_FOUND, { error: "file could not be read" });
        return;
      }
      if (!st.isFile()) {
        closeSync(fd);
        json(res, HTTP_BAD_REQUEST, { error: "not a file" });
        return;
      }
    ...
    "GET /file-content": (req, res, url) => {
      ...
      if (!target) { forbidden(res, url.searchParams.get("path")); return; }
      let st;
      try { st = statSync(target); } catch (e) { json(res, HTTP_NOT_FOUND, { error: e.message }); return; }
      if (!st.isFile()) { json(res, HTTP_BAD_REQUEST, { error: "not a file" }); return; }
      if (st.size > MAX_EDITABLE_FILE_SIZE) { json(res, HTTP_CONTENT_TOO_LARGE, { error: `file too large to edit in browser (${st.size} bytes)` }); return; }
      let buf;
      try { buf = readFileSync(target); }
      catch { json(res, HTTP_NOT_FOUND, { error: "file could not be read" }); return; }
      // Recheck after reading because a file can grow between stat and read.
      if (buf.length > MAX_EDITABLE_FILE_SIZE) { json(res, HTTP_CONTENT_TOO_LARGE, { error: `file too large to edit in browser (${buf.length} bytes)` }); return; }
      if (buf.includes(0)) { json(res, HTTP_UNSUPPORTED_MEDIA_TYPE, { error: "binary file — download it instead" }); return; }
      json(res, HTTP_OK, { path: target, content: buf.toString("utf8") });
    },
    ...
    "POST /file-save": async (req, res) => {
      ...
      if (!body || typeof body !== "object" || Array.isArray(body) || typeof body.path !== "string") {
        json(res, HTTP_BAD_REQUEST, { error: "path must be a string" });
        return;
      }
      const target = resolveSafePath(resolve(body.path));
      if (!target) { forbidden(res, body.path); return; }
      if (typeof body.content !== "string") { json(res, HTTP_BAD_REQUEST, { error: "content must be a string" }); return; }
      let dirOk = false;
      try { dirOk = statSync(dirname(target)).isDirectory(); } catch {}
      if (!dirOk) { json(res, HTTP_BAD_REQUEST, { error: `no such directory: ${dirname(target)}` }); return; }
      ...
      try {
        ...
        json(res, HTTP_INTERNAL_SERVER_ERROR, { error: "save failed" });
        return;
      }
      log(`[oyster] file saved via explorer: ${target}`);
      json(res, HTTP_OK, { saved: target, bytes: Buffer.byteLength(body.content) });
    },
    ...
    "POST /mkdir": async (req, res) => {
      ...
      if (!body || typeof body !== "object" || Array.isArray(body)
        ...
        json(res, HTTP_BAD_REQUEST, { error: "path and folder name must be strings" });
        return;
      }
      ...
      if (!name || name === "." || name === ".." || /[/\\]/.test(name)) {
        json(res, HTTP_BAD_REQUEST, { error: "invalid folder name" });
        return;
      }
      let parentOk = false;
      try { parentOk = statSync(parent).isDirectory(); } catch {}
      if (!parentOk) { json(res, HTTP_BAD_REQUEST, { error: `not a directory: ${parent}` }); return; }
      const target = join(parent, name);
      if (existsSync(target)) { json(res, HTTP_CONFLICT, { error: `already exists: ${target}` }); return; }
      try { mkdirSync(target); }
      catch (error) { json(res, HTTP_INTERNAL_SERVER_ERROR, { error: `mkdir failed: ${error.message}` }); return; }
      log(`[oyster] created folder ${target}`);
      json(res, HTTP_CREATED, { path: target });
    },
  };
}

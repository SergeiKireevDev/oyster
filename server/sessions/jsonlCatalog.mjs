const SESSION_PREVIEW_MAX_CHARS = 120;
const ENTRY_LABEL_MAX_CHARS = 200;
...
export function summarizeSessionFile(path) {
  ...
  return {
    ...
    preview: firstUserText?.slice(0, SESSION_PREVIEW_MAX_CHARS) ?? null,
    messageCount,
  };
}
...
function searchEntry(entry, { terms, operator, includeTools, meta }) {
  ...
  for (const part of entryTexts(entry)) {
    if (!meta.preview && part.role === "user" && part.kind === "text") meta.preview = part.text.slice(0, SESSION_PREVIEW_MAX_CHARS);
...
export function searchSessions({ q, scope, path, includeTools = false, defaultDir = null } = {}, maxResults = 200) {
  ...
  for (const file of files) {
    const hits = searchSessionFile(file, parsedQuery, MAX_SEARCH_HITS, includeTools);
...
function treeNodeLabel(entry) {
  if (entry.type === "message") return (labelOf(entry.message ?? {}) ?? "").slice(0, ENTRY_LABEL_MAX_CHARS);
...
export function sessionEntries(path) {
  ...
  for (const e of chain) {
    ...
    out.push({ id: e.id, role: m.role, text: (labelOf(m) ?? "").slice(0, ENTRY_LABEL_MAX_CHARS), timestamp: e.timestamp ?? null });
  }

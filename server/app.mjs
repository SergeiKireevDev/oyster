...
export async function buildCandidate(stableState, { generation = Symbol("application-candidate") } = {}) {
  ...
  const { createPiCredentialService } = await import(bust("pi-credential-service.mjs")); const { createClaudeOAuthCredentialSink } = await import(bust("claude-oauth-credential-sink.mjs")); const { createClaudeOAuthRefreshService } = await import(bust("claude-oauth-refresh-service.mjs")); const { createPiOAuthFlowService } = await import(bust("pi-oauth-flow-service.mjs")); const { createRestartActiveRunners } = await import(bust("runner-restart-service.mjs"));
  ...
  if (claudeOAuthCredentialSink) { claudeOAuthRefresh = createClaudeOAuthRefreshService({ rotate: (options) => credentialService.rotateAnthropicOAuth(options), restartRunners: () => restartActiveRunners({ harness: "claude-code", idleOnly: true }), setTimer: (callback, delay) => setTimeout(scope.guard(callback), delay) }); claudeOAuthRefresh.start(); scope.defer(() => claudeOAuthRefresh.stop()); }

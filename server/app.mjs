...
export async function buildCandidate(stableState, { generation = Symbol("application-candidate") } = {}) {
  ...
  if (claudeOAuthCredentialSink) { claudeOAuthRefresh = createClaudeOAuthRefreshService({ sink: claudeOAuthCredentialSink, restartRunners: () => restartActiveRunners({ harness: "claude-code", idleOnly: true }), setTimer: (callback, delay) => setTimeout(scope.guard(callback), delay) }); claudeOAuthRefresh.start(); scope.defer(() => claudeOAuthRefresh.stop()); }

...
export async function buildCandidate(stableState, { generation = Symbol("application-candidate") } = {}) {
  ...
  const tunnelRoutes = createTunnelRoutes({
    state, appStore, config, requestContext, listTunnels, reserveHublot, recordHublotTransition, rebindHublot, openTunnel, closeTunnel, reopenHublot,
    ensureSessionOwner,
  ...
  const codexOAuthCredentialSink = config.CODEX_BIN ? createCodexOAuthCredentialSink({ configDir: config.CODEX_HOME }) : null; const geminiOAuthCredentialSink = config.GEMINI_BIN ? createGeminiOAuthCredentialSink({ credentialPath: config.GEMINI_OAUTH_PATH, geminiBin: config.GEMINI_BIN }) : null;

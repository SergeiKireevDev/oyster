...
- **Checkpoints** — an iceberg button (🧊) rides on the latest message of the transcript; tapping it commits every pending change in the session's workdir (`git add -A && git commit`) as a `checkpoint …` commit, freezing the state the conversation reached at that point. If the workdir is clean (or not a git repo) it says so instead.

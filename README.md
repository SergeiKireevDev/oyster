...
- **Checkpoints** — an iceberg button (🧊) rides on the latest message of the transcript; tapping it commits every pending change in the session's workdir (`git add -A && git commit`) as a `checkpoint …` commit, freezing the state the conversation reached at that point (a clean workdir is marked at HEAD instead). Checkpoints are anchored to the message they were taken at and persisted in `~/.pi/agent/checkpoints.json`. Every checkpointed message then carries a return arrow (↩): tapping it deterministically rolls the workdir back to that commit — pending changes are auto-committed first, so nothing is ever lost, then `git reset --hard` — and automatically opens a **forked session** whose history ends at that message (the fork inherits the ancestors' checkpoint markers, so you can hop between states freely, including back "forward" to the auto-saved tip). No LLM is involved anywhere. The 🌳 header chip toggles a right sidebar visualising the whole family as a tree: the session's root ancestor, every fork nested under the checkpoint it was created from (🌱 root · 🌿 forks, with live/busy dots), and each session's 🧊 checkpoints — tap a session to switch to it, tap a checkpoint to roll back and fork right from the sidebar. The sessions picker keeps things tidy too: forked sessions are collapsed under their main session (🌿 *n forks*), and a whole fork family counts as “active” if any member has a live process.

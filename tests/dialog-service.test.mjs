...
test("text and editor prompt bodies and footers consume the scoped dialog service", () => {
  ...
  const optionItem = readFileSync(new URL("../public/src/components/OptionPickerItem.svelte", import.meta.url), "utf8");
  ...
  assert.match(modal, /<form class="text-prompt" onsubmit=\{submitTextPrompt\}>/);
  ...
  assert.match(editor, /<form\b[^>]*onsubmit=\{submitEditorPrompt\}>/);
  ...
  assert.match(option, /<OptionPickerItem/);
  assert.match(optionItem, /model-autocomplete-option/);

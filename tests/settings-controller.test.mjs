...
test("settings controller selects and applies a model", async () => {
  ...
  const controller = createSettingsController({ rpc: async (request) => { calls.push(request); return request.type === "get_available_models" ? { models: [{ provider: "p", id: "m" }] } : {}; }, pickOption: async () => 0, refreshState: () => {}, toast: (...args) => toasts.push(args) });

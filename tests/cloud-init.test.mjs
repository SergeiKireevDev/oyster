...
test("cloud-init installs Oyster from the requested source and starts its outbound box agent", () => {
  ...
  assert.match(files.get("/usr/local/sbin/install-oyster-box"), /systemctl enable --now oyster\.service oyster-box-agent\.service/);
  assert.match(files.get("/etc/systemd/system/oyster-box-agent.service"), /ExecStart=\/usr\/bin\/node \/opt\/oyster\/oyster-hub\/box-agent\.mjs/);

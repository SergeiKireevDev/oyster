...
function startContainer() {
  ...
  sh(`docker run -d --name ${container} -p "${allocatedPort}:4000" -v "${agentVolume}:/root/.pi/agent" -e "OYSTER_TOKEN=${TOKEN}" -e "E2E_MOCK_LLM=1" -e "E2E_MOCK_TUNNELS=1" -e "CLAUDE_CODE_BIN=/opt/mock-claude-code.mjs" -e "OYSTER_SKIP_PUBLIC_HUBLOT_READINESS=1" -e "PERSISTENT_STORE=${selectedStore}" ${selectedImage}`);
}

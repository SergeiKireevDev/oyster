...
test("supervisor reconciles every desired-open hublot against persisted identities", async (t) => {
  ...
  assert.equal((await store.repositories.hublots.find(stale.id)).status, "recovering");
  assert.equal((await store.repositories.hublots.find(stale.id)).public_url, null);
  assert.match((await store.repositories.hublots.find(stale.id)).last_error, /tunnel process identity is not live/);
...
test("repeated service failure uses bounded backoff and crash-loop protection instead of unbounded spawning", async (t) => {
  const { store, state } = await fixture(t);
  const hublot = await reserveHublot(state, { port: 4221, brief: "managed service" });
  await recordHublotTransition(state, hublot.id, "open", { publicUrl: "https://crashing.test" });
  let time = Date.parse("2026-01-01T00:00:00.000Z");
  let restartAttempts = 0;
  const options = {
    appStore: store,
    recordTransition: async (id, status, details) => await recordHublotTransition(state, id, status, details),
  ...
  assert.equal((await store.repositories.hublots.find(hublot.id)).restart_count, 1);
  assert.equal((await store.repositories.hublots.find(hublot.id)).next_restart_at, "2026-01-01T00:00:00.100Z");
  ...
  assert.equal((await store.repositories.hublots.find(hublot.id)).restart_count, 2);
  assert.equal((await store.repositories.hublots.find(hublot.id)).next_restart_at, "2026-01-01T00:00:00.250Z");

  time += 150;
  const limited = await supervisor.reconcile();
  assert.equal(restartAttempts, 3);
  assert.equal(limited.crashLooped, 1);
  assert.equal((await store.repositories.hublots.find(hublot.id)).status, "interrupted");
  assert.equal((await store.repositories.hublots.find(hublot.id)).restart_count, 3);
  assert.equal((await store.repositories.hublots.find(hublot.id)).next_restart_at, null);
  assert.match((await store.repositories.hublots.find(hublot.id)).last_error, /automatic restart disabled after 3 consecutive failures/);

  const historyAtCutoff = (await store.repositories.hublots.listLifecycleEvents(hublot.id)).length;
  for (let pass = 0; pass < 100; pass++) {
    time += 60_000;
    await supervisor.reconcile();
  }
  assert.equal((await store.repositories.hublots.listLifecycleEvents(hublot.id)).length, historyAtCutoff, "cutoff does not append unbounded transition history");
  assert.equal(restartAttempts, 3, "crash-looped hublots must not spawn again even after backoff deadlines pass");
  assert.equal((await store.repositories.hublots.find(hublot.id)).desired_state, "open", "cutoff remains durable until an operator intervenes");
});

test("supervisor does not overwrite a manual close during an asynchronous health check", async (t) => {
  const { store, state } = await fixture(t);
  const hublot = await reserveHublot(state, { port: 4203, serviceKind: "self_served" });
  ...
  await checkStarted;
  await recordHublotTransition(state, hublot.id, "closed", { desiredState: "closed", publicUrl: null, closedAt: "closed" });
  resolveCheck(false);
...
test("supervisor ignores stale health results after concurrent process recovery", async (t) => {
  const { store, state } = await fixture(t);
  const hublot = await reserveHublot(state, { port: 4205, serviceKind: "self_served" });
  await recordHublotTransition(state, hublot.id, "open", { publicUrl: "https://live.test" });
  let checkStarted;
  ...
  const reconciliation = supervisor.reconcile();
  await started;
  await processRow(store, hublot.id, "replacement-tunnel", "tunnel");
  finishCheck(false);
  ...
  assert.equal(recoveryAttempts, 0);
  assert.equal((await store.repositories.hublots.find(hublot.id)).status, "open");
  assert.equal((await store.repositories.hublots.find(hublot.id)).public_url, "https://live.test");
});

test("supervisor does not turn a concurrently closed hublot into a restart failure", async (t) => {
  const { store, state } = await fixture(t);
  const hublot = await reserveHublot(state, { port: 4204, brief: "managed service" });
  await recordHublotTransition(state, hublot.id, "open", { publicUrl: "https://restart-race.test" });
  let restartStarted;
  ...
  const result = await reconciliation;
  const closed = await store.repositories.hublots.find(hublot.id);
...
test("failed tunnel recovery does not overwrite a concurrent successful open", async (t) => {
  const { store, state } = await fixture(t);
  const hublot = await reserveHublot(state, { port: 4206, serviceKind: "self_served" });
  await recordHublotTransition(state, hublot.id, "open", { publicUrl: "https://old.test" });
  let recoveryStarted;
  ...
  const reconciliation = supervisor.reconcile();
  await started;
  await recordHublotTransition(state, hublot.id, "open", { publicUrl: "https://replacement.test" });
  rejectRecovery(new Error("stale recovery failed"));
  const report = await reconciliation;
  const reopened = await store.repositories.hublots.find(hublot.id);

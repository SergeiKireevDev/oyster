...
test("stopped-service import plans then applies checkpoints, routine definitions, and bindings", async (t) => {
  ...
  const applied = await importLegacyAppData({
    ...
    checkpointSourcePath: checkpointPath, routineSourceDir: routinesDir, id: "apply", now: () => "apply-time",
  });
  ...
  assert.equal(readFileSync(checkpointPath, "utf8"), checkpointSource, "import leaves checkpoint source untouched");
  assert.equal(readFileSync(join(routinesDir, "bindings.json"), "utf8"), bindingsSource, "import leaves bindings untouched");

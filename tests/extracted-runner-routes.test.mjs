...
test("app composes the extracted runner factory with explicit operations", () => {
  assert.match(appSource, /createRunnerRoutes\(\{[\s\S]*?state, requestContext, runnerFromReq, startRunner, listRunnerInfo,[\s\S]*?sessionReferenceParam,[\s\S]*?lookupSessionReference:[\s\S]*?srvId, runnersChanged,[\s\S]*?\}\)/);

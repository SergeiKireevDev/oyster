...
test("session query partitions stopped sessions by two-day head age", () => {
  ...
  const archivedWhileAlive = { sessionKey: "archived-alive", cwd: "/work", modifiedAt: "2026-07-17T11:59:00.000Z", archived: true };
  ...
  const groups = [{ cwd: "/work", entries: [
    ...
    { session: archivedWhileAlive, runner: aliveRunner },
  ] }];
  ...
  assert.equal(isSessionEntryArchived(groups[0].entries[4], now), true);
  assert.deepEqual(partitionSessionGroupsByArchive(groups, now), [
    ...
    { cwd: "/work", entries: [groups[0].entries[1], groups[0].entries[3], groups[0].entries[4]], archived: true, firstArchived: true },
  ]);
});

...
const service = readFileSync(new URL("../oyster.service", import.meta.url), "utf8");
...
test("systemd template pins the local SQLite-enabled pi runtime", () => {
  assert.match(service, /Environment=PI_BIN=__OYSTER_DIR__\/pi\/packages\/coding-agent\/dist\/cli\.js/);
  assert.match(service, /Environment=PERSISTENT_STORE=sqlite/);
  assert.match(service, /Environment=PI_CODING_AGENT_DIR=%h\/\.pi\/agent/);
  assert.match(service, /WorkingDirectory=__OYSTER_DIR__/);
});

test("service documentation installs, verifies, updates, and rolls back the service", () => {
  assert.match(serviceDocs, /sed "s\|__OYSTER_DIR__\|\$\(pwd\)\|g" oyster\.service/);
  assert.match(serviceDocs, /systemctl --user restart oyster\.service/);

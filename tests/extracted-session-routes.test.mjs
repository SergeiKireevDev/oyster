...
test("app composes session routes with explicit domain dependencies", () => {
  assert.match(appSource, /createSessionRoutes\(\{[\s\S]*?sessions: \{[\s\S]*?catalog: state\.sessionCatalog,[\s\S]*?sessionTargetFromSearch,[\s\S]*?runners: \{ stopRunner, runnersChanged \},[\s\S]*?resources: \{ closeTunnel, releaseSessionRoutines \},[\s\S]*?\}\)/);

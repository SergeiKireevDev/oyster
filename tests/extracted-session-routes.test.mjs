...
test("app composes session routes with explicit domain dependencies", () => {
  assert.match(appSource, /createSessionRoutes\(\{[\s\S]*?sessions: \{[\s\S]*?searchSessions[\s\S]*?sessionFileFromSearch,[\s\S]*?runners: \{ stopRunner, runnersChanged \},[\s\S]*?resources: \{ closeTunnel, releaseSessionRoutines \},[\s\S]*?\}\)/);

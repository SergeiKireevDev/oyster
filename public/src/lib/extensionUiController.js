...
export function createExtensionUiController({ respond, toast, confirm, select, input, editor, setTitle }) {
  return async (request) => {
    switch (request.method) {
      ...
      case "input": { const value = request.secret === true
        ? await input(request.title, request.placeholder, "", { secret: true })
        : await input(request.title, request.placeholder); return respond(request.id, value == null ? { cancelled: true } : { value }); }
